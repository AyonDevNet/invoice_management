...
# JWT error handlers — return 401 with JSON so frontend gets consistent auth errors (not 422)
@jwt.unauthorized_loader
def unauthorized_callback(reason):
    return jsonify({'error': 'Authorization required', 'message': reason or 'Missing or invalid token'}), 401


@jwt.invalid_token_loader
def invalid_token_callback(reason):
    return jsonify({'error': 'Invalid token', 'message': reason or 'Token is invalid or malformed'}), 401
...
def revoked_token_callback(jwt_header, jwt_payload):
    return jsonify({'error': 'Token revoked', 'message': 'Please sign in again'}), 401
